...
const TestGridSpace = ({ space, onExit }: TestGridSpaceProps): ReactElement => {
  ...
  // Single player state - we'll get the actual display name from WebSocket
  ...
  // Update player name when we get the display name from WebSocket
  useEffect(() => {
    if (currentUserDisplayName && currentUserDisplayName !== "You" && currentUserDisplayName !== "Loading...") {
